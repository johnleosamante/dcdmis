...
if (!is_dir('../../pcdmis/training/'.$_GET['code'])) {
    mkdir('../../pcdmis/training/'.$_GET['code'], 0777, true);
}
        ...
        uploader.init();
			...
			<div class="col-lg-12">
						          <div class="panel panel-default">
									...
									<a href="./?7e9ff1f60111f1bf6a3696b2092ac4a7285cd9427e9ff1f60111f1bf6a3696b2092ac4a7285cd942&v=bGlzdF9vZl9hY3Rpdml0eQ%3D%3D" class="btn btn-secondary" style="float:right;padding:4px;margin:4px;">Back</a>
									<a href="#newca" class="btn btn-warning" style="float:right;padding:4px;margin:4px;" data-toggle="modal">Upload CA</a>
									<a href="#newtemplate" class="btn btn-primary" style="float:right;padding:4px;margin:4px;" data-toggle="modal">Upload Certificate</a>
						          		<h4>List of Participants</h4>
											...
											}elseif (isset($_POST['uploadCA']))
											{
												if (!is_dir('../../pcdmis/training/CA/'.$_GET['code'])) {
													mkdir('../../pcdmis/training/CA/'.$_GET['code'], 0777, true);
												}
												$myfile = $_FILES['uploadca']['name'];
												//$myfile = preg_replace("/[^a-zA-Z0-9-.]/", "", $myfile);
												$temp = $_FILES['uploadca']['tmp_name'];
												$type = $_FILES['uploadca']['type'];
												$ext = pathinfo($myfile, PATHINFO_EXTENSION);	
												$mynewimage='../../pcdmis/training/CA/'.$_GET['code'].'/'.$_GET['code'].'.'.$ext;
												move_uploaded_file($temp, $mynewimage);
												
												mysqli_query($con,"UPDATE tbl_seminar SET CertifacateofApperance='".$mynewimage."' WHERE Training_Code ='".$_GET['code']."' LIMIT 1");
											...
											<?php
											echo '<a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&v='.urlencode(base64_encode("new_participants")).'" style="float:right;padding:4px;margin:4px;" class="btn btn-success">Add Participant</a>';
											echo '<a href="printca.php?code='.urlencode(base64_encode($_GET['code'])).'" style="float:right;padding:4px;margin:4px;" class="btn btn-danger" target="_blank">Print CA</a>';
									...
									while($viewrow=mysqli_fetch_array($viewparticipants))
									{
										...
										echo '<tr>
												...
												<td>
												<a href="print_certificate.php?certificate='.urlencode(base64_encode($viewrow['Emp_ID'])).'" target="_blank">Print</a> <br/> 
...
<div class="panel-body">
                 ...
                 <!-- Modal -->
	 <div class="modal fade" id="newca" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	 <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" onclick="window.location.reload();">&times;</button>
          <h3 class="modal-title"><center>UPLOAD CA</center></h3> 	
        </div>
		<form action="" Method="POST" enctype="multipart/form-data">
		 <div class="modal-body">
		 <img src="" style="width:100%;height:300px;padding:4px;margin:4px;" id="pic1"><br/><br/>
		  
		  <center>
		<div id="container">									
		 <input type="file" name="uploadca" onchange="loadFile1(event);">
		</div>
		 ...
		 <div class="modal-footer">
		 <input type="submit" name="uploadCA"  value="Upload CA" class="btn btn-success">
		 </div>
		 </form>
        </div>
        </div>
