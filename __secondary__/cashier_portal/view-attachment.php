...
$url=$_GET[$key]=base64_decode(urldecode($data));
	...
	$posted=mysqli_query($con,"SELECT * FROM post INNER JOIN tbl_employee ON post.posted_by=tbl_employee.Emp_ID  WHERE post.chatID = '".$url."' ORDER BY post.date_posted Desc");
	$row=mysqli_fetch_assoc($posted);
	$_SESSION['chat-id']=$url;
