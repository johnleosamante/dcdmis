...
//$connect=mysqli_connect("depedpagadian.org","root","Marlon@1986","pcdmis");
...
$img2='../../pcdmis/logo/logo.png';
	...
	$mydtrinfo=mysqli_query($con,"SELECT * FROM tbl_employee INNER JOIN tbl_station ON tbl_employee.Emp_ID =tbl_station.Emp_ID INNER JOIN tbl_school ON tbl_station.Emp_Station = tbl_school.SchoolID INNER JOIN tbl_job ON tbl_station.Emp_Position=tbl_job.Job_code WHERE  tbl_station.Emp_Station='123131' AND tbl_employee.Emp_Status = 'Active' ORDER BY Emp_LName Asc");
while($rowdtr=mysqli_fetch_array($mydtrinfo))
{
    //ofcourse we need rights to create temp dir
    if (!file_exists($PNG_TEMP_DIR))
        mkdir($PNG_TEMP_DIR);
     $finame = $PNG_TEMP_DIR.$rowdtr['Emp_ID'].'.png';
    //remember to sanitize user input in real-life solution !!!
    $errorCorrectionLevel = 'L';
    $matrixPointSize = 5;
   $_REQUEST['data']=$rowdtr['Emp_ID'];
...
//Next collunm
$pdf->Image($img3,15,195,15);
$pdf->Image($img3,105,195,15);
$pdf->Image($img3,195,195,15);
$pdf->Image($img3,285,195,15);
...
//Header Information
	...
	$myname=mysqli_query($con,"SELECT * FROM tbl_employee WHERE Emp_ID='".$rowdtr['Emp_ID']."' AND Emp_Status = 'Active'  LIMIT 1");
	...
	//Data started
	$myinfoDTR=mysqli_query($con,"SELECT * FROM tbl_dtr WHERE tbl_dtr.DTRDate BETWEEN '".$_POST['date_from']."' AND '".$_POST['date_to']."' AND tbl_dtr.Emp_ID='".$rowdtr['Emp_ID']."' ORDER By DTRDate Asc");
	...
	//Signaturies
	$myoffice=mysqli_query($con,"SELECT * FROM tbl_station WHERE Emp_ID ='".$rowdtr['Emp_ID']."' LIMIT 1");
	...
	//First Line 
	$pdf->Cell(0,5,"",0,1,'C');
    $pdf->Cell(70,3,$myincharge['Office_Chief'],0,0,'C');
	...
	//Fourth Line 
	$pdf->Cell(20,3,"",0,0,'C');
	$pdf->Cell(70,3,$myincharge['Office_Chief'],0,1,'C');
	...
	//First Line
    $pdf->Cell(70,3,$myincharge['Position'],0,0,'C');
	...
	//Fourth Line 
	$pdf->Cell(20,3,"",0,0,'C');
	$pdf->Cell(70,3,$myincharge['Position'],0,1,'C');
...
//$pdf->Cell(0,2,"",0,1,'C');
//$code=$rowdtr['Emp_ID'];
$code="System Generated as of ".$dateposted;	

$pdf->Code128(30,196,$code,50,12);
$pdf->Code128(120,196,$code,50,12);
$pdf->Code128(210,196,$code,50,12);
$pdf->Code128(300,196,$code,50,12);
