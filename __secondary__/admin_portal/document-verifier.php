	<script>
		function viewdata(str){
		  ...
		  xmlhttp.open("GET","view_destiny.php?id="+str,false);
		  xmlhttp.send();
		}
	</script> 
 <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
						...
						if (isset($_POST['dts']))
							...
							{
								if ($_POST['officeTo']=='ITO COPY')
								{	
								  mysqli_query($con,"UPDATE tbl_transactions SET Trans_Stats='Completed' WHERE TransCode='".$_SESSION['Transcode']."' LIMIT 1");	
								}else{
							...
							if(mysqli_affected_rows($con)==1)
								{
									...
									if ($_POST['officeTo']=='ITO COPY')
									{	
									mysqli_query($con,"INSERT INTO tbl_transactions_log VALUES(NULL,'".$dateposted."','".$_SESSION['uid']."','".$_SESSION['station']."','-','Completed','".$_SESSION['Transcode']."','New')");
									}else{
								...
								echo '<div class="alert alert-success">'.$Err.'<script>{window.location.href="./?13b714fad9eca2a00fe69ce8ce03cba1c7e085277e9ff1f60111f1bf6a3696b2092ac4a7285cd942&v=ZGFzaGJvYXJk";}</script></div>';
								$_SESSION['Transcode']="";
									}else{
                            ...
                            <?php
									 ...
									 $row=mysqli_fetch_assoc($mystatus);
										...
										elseif ($row['Trans_Stats']=="For release")
											{
												echo '<a href="done.php?id='.$row['TransCode'].'&From='.$row['Trans_from'].'" class="btn btn-success"  style="float:right;"></i>Click to Complete</a>';
											
											}
											elseif ($row['Trans_Stats']<>"On Process")
											{
											echo '<a href="view-recieve.php?id='.$row['TransCode'].'" data-toggle="modal" data-target="#Mylog" class="btn btn-success"  style="float:right;"></i>Click to receive</a>';
											}else{
											echo '<a href="confirm.php?id='.$row['TransCode'].'" data-toggle="modal" data-target="#Mylog" class="btn btn-success"  style="float:right;">Click Next</a>';	
											}
