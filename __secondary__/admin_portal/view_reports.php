...
$_SESSION['SubCode']=$code;
	...
	//Query for subject
	$subject=mysqli_query($con,"SELECT * FROM tbl_assessment_rat_subject WHERE tbl_assessment_rat_subject.RATSubCode ='".$code."' AND tbl_assessment_rat_subject.Exam_Status='".$_SESSION['rat_status']."' AND tbl_assessment_rat_subject.Grade_Level = '".$_SESSION['GLevel']."' LIMIT 1");
	...
	//Query for Learner
	$learner=mysqli_query($con,"SELECT * FROM tbl_assessment_rat WHERE tbl_assessment_rat.YLevel='".$_SESSION['GLevel']."' AND tbl_assessment_rat.ExamStatus='DONE'");
	...
	$pdf->Cell(60,5,strtoupper($rowsub['Learning_Areas']).' '.$_SESSION['GLevel'],0,1);
	//List of Learner
	$pdf->Cell(0,2,'',0,1);
	...
	$pdf->Cell(15,5,$row['QNumber'],1,0,'C');	
    $xPos=$pdf->GetX();
	$yPos=$pdf->GetY();
	$pdf->multiCell(85,5,$row['Questionnairs'],1);	
	$pdf->SetXY($xPos+85,$yPos);
	$pdf->cell(25,5,$row['NoOfCorrect'],1,0,'C');
	//$pdf->SetXY($xPos+110,$yPos);
	$pdf->cell(20,5,number_format($percent,0).'%',1,0,'C');
	//$pdf->SetXY($xPos+130,$yPos);
	$pdf->cell(53,5,$remarkdata,1,1,'C');
	...
	$pdf->Cell(20,5,number_format($TotalPercent,2).'%',1,0,'C');
	$pdf->Cell(53,5,$remarkdata,1,1,'C');
	$pdf->Cell(0,5,"",0,1,'C');
	...
	$pdf->Cell(30,5,'Grade '.$_SESSION['GLevel'],1,0,'C');
	...
	$pdf->Cell(30,5,'JOEL D. BATERNA',0,1);
