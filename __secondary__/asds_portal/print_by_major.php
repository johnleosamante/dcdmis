...
$url=$_GET[$key]=base64_decode(urldecode($data));
		...
		require_once '../Classes/PHPExcel.php';
	     ...
	     $filename = $url.' Major'; //your file name
		...
		//Get data from the database
			...
			if ($url=='SCIENCE')
			{
			 $result=mysqli_query($con,"SELECT * FROM tbl_applicant INNER JOIN tbl_applicant_rating ON tbl_applicant.Appl_No=tbl_applicant_rating.ApplicanNo WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Major LIKE '%".$url."%' AND  tbl_applicant_rating.OverALL >=70 ORDER BY tbl_applicant_rating.OverALL Desc");
			}else{	
			  $result=mysqli_query($con,"SELECT * FROM tbl_applicant INNER JOIN tbl_applicant_rating ON tbl_applicant.Appl_No=tbl_applicant_rating.ApplicanNo WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Major='".$url."' AND tbl_applicant_rating.OverALL >=70 ORDER BY tbl_applicant_rating.OverALL Desc");
			}
            ...
            //Rating data
				...
				$objPHPExcel->setActiveSheetIndex(0) 
					...
					->setCellValue('E'.$no, $row['One'])
					->setCellValue('F'.$no, $row['EdEquiv'])	
					->setCellValue('G'.$no, $row['Three'])	
					->setCellValue('H'.$no, $row['EdSubTotal'])	
					->setCellValue('I'.$no, $row['Five'])	
					->setCellValue('J'.$no, $row['Six'])	
					->setCellValue('K'.$no, $row['TeachSubTotal'])	
					->setCellValue('L'.$no, $row['Eight'])	
					->setCellValue('M'.$no, $row['RateEquive'])
					->setCellValue('N'.$no, $row['Ten'])
					->setCellValue('O'.$no, $row['Eleven'])
					->setCellValue('P'.$no, $row['SpecialSubTotal'])
					->setCellValue('Q'.$no, $row['Thirteen'])
					->setCellValue('R'.$no, $row['Fourteen'])
					->setCellValue('S'.$no, $row['Fifteen'])
					->setCellValue('T'.$no, $row['EngEval'])
					->setCellValue('U'.$no, $row['OverALL'])
					;	
