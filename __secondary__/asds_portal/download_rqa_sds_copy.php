<?php
    ...
    include("../../pcdmis/vendor/jquery/function.php");
		...
		require_once '../Classes/PHPExcel.php';
	     ...
	     $filename = "OSDS_RQA_COPY"; //your file name
		...
		//Get data from the database
			$no=15;
			  ...
			  $result=mysqli_query($con,"SELECT * FROM tbl_applicant INNER JOIN tbl_applicant_rating ON tbl_applicant.Appl_No=tbl_applicant_rating.ApplicanNo WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Category='".$_SESSION['ApplicantCat']."' AND tbl_applicant_rating.OverALL >= '70' AND tbl_applicant.Major LIKE '%"."SCIENCE"."%' ORDER BY tbl_applicant_rating.OverALL Desc");
			...
			unset($styleData);	
				...
				$objPHPExcel->setActiveSheetIndex(0) 
					->setCellValue('A'.$no,utf8_decode($row['Last_Name'].', '.$row['First_Name'].' '.$MName.'.'))
					->setCellValue('B'.$no, strtoupper($row['Major']))
					->setCellValue('C'.$no, $row['Contact_No'])
					->setCellValue('D'.$no, utf8_decode($row['Home_Address']))
		...
		header("Content-Disposition: attachment;Filename=OSDS_RQA_COPY.xls");
