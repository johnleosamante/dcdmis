                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
						...
						if (isset($_POST['dts']))
							...
							if(mysqli_affected_rows($con)==1)
								{
									if ($_POST['status']=='ASDS Copy')
									{
									mysqli_query($con,"INSERT INTO tbl_transactions_log VALUES(NULL,'".$dateposted."','".$_SESSION['uid']."','".$_SESSION['station']."','-','".$_POST['status']."','".$_SESSION['Transcode']."','Done')");
									}else{
                            ...
                            <?php
									 ...
									 $row=mysqli_fetch_assoc($mystatus);
										if ($row['Trans_Stats']=="Completed")
											{
												echo '<a href="print-report.php?id='.urlencode(base64_encode($row['TransCode'])).'&From='.$row['Trans_from'].'" class="btn btn-success" target="_blank" style="float:right;">Click to Print</a>';
											
											}
										elseif ($row['Trans_Stats']=="For release")
											{
												echo '<a href="done.php?id='.urlencode(base64_encode($row['TransCode'])).'&From='.$row['Trans_from'].'" class="btn btn-success"  style="float:right;"></i>Click to Complete</a>';
