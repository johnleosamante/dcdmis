...
include("../../../pcdmis/vendor/jquery/function.php");
		...
		require_once '../../Classes/PHPExcel.php';
	     ...
	     $filename = 'Print OSDS Copy'; //your file name
		...
		//Get data from the database
			$no=15;
			  ...
			  $result=mysqli_query($con,"SELECT * FROM tbl_applicant INNER JOIN tbl_applicant_rating ON tbl_applicant.Appl_No=tbl_applicant_rating.ApplicanNo WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Major='"."ARAL-PAN"."' ORDER BY tbl_applicant.Last_Name Asc");
		...
		$objPHPExcel->setActiveSheetIndex(0) 
		 ...
		 //End of araling Panlipunan
		 
		 
		 
		 
		 
		 //Start Second Subject English
		
		 $no=$no+5;
		 $noto=$no+5;
		 
		//First Cells 
		$objPHPExcel->getActiveSheet()->getStyle('A'.$no.':A'.$noto)->getAlignment()->setHorizontal('center');
		$objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'.$no.':U'.$no);	
		$objPHPExcel->getActiveSheet()->getStyle('A'.$no.':A'.$no)->getFont()->setBold(true)
                                ->setName('Old English Text MT')
                                ->setSize(11)
                                ->getColor()->setRGB('#000000');
										
		$objPHPExcel->setActiveSheetIndex(0) 
			->setCellValue('A'.$no, 'Republic of the Philippines');
		...
		$objPHPExcel->setActiveSheetIndex(0) 
			->setCellValue('A'.$no, 'Department of Education');	
