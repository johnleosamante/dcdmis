...
//$pdf->Image($img4,90,240,120);
	...
	//Record
	$no=0;
	$list=mysqli_query($con,"SELECT * FROM tbl_pisa_participant WHERE SchoolID='".$_SESSION['school_id']."'");
	while($row=mysqli_fetch_array($list))
	{
	$no++;
	$myscore=mysqli_query($con,"SELECT * FROM tbl_pisa_monitoring WHERE tbl_pisa_monitoring.SchoolID='".$_SESSION['school_id']."' AND tbl_pisa_monitoring.LRN='".$row['LRN']."' AND tbl_pisa_monitoring.date_taken='".$_SESSION['CurrentDate']."' AND SubCode='".$_SESSION['SubCode']."'LIMIT 1");
	//$myscore=mysqli_query($con,"SELECT * FROM tbl_pisa_monitoring WHERE tbl_pisa_monitoring.SchoolID='".$_SESSION['school_id']."' AND tbl_pisa_monitoring.LRN='".$row['LRN']."' AND tbl_pisa_monitoring.date_taken='".$_SESSION['CurrentDate']."' AND SubCode='12345625'LIMIT 1");
