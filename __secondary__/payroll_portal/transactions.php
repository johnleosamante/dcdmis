                         ...
                         <div class="panel-heading">
						  <a href="#newtrack" class="btn btn-primary" style="float:right;" data-toggle="modal">New Transaction</a>
							...
							if (isset($_POST['new_transaction']))
							{
								...
								mysqli_query($con,"INSERT INTO tbl_transactions VALUES ('".$_POST['Qualicode']."','".$dataA	."','".$dateposted."','PAYROLL','".$_POST['status']."','Unread','123131','')");
								if (mysqli_affected_rows($con)==1)
								{
									mysqli_query($con,"INSERT INTO tbl_transactions_log VALUES (NULL,'".$dateposted."','".$_SESSION['uid']."','PAYROLL','".$_POST['first']."','".$_POST['status']."','".$_POST['Qualicode']."','New')");
													...
													<td style="text-align:center;">';
														if ($row['Trans_Stats']=='Completed' || $row['Trans_Stats']=='COMPLETED')
														{
														 echo '<a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&id='.urlencode(base64_encode($row['TransCode'])).'&v='.urlencode(base64_encode("quatame")).'" title="QUATAME"> Evaluation</a>';
														  ...
														  echo '<a href="print-transaction.php?link='.sha1("Deped Data Management System v.1.0").'&&Code='.urlencode(base64_encode($row['TransCode'])).'" target="_blank" title="Print Transaction" class="btn btn-info" style="padding:4px;margin:4px;"><i class="fa  fa-print fa-fw"></i></a>';		
														}
														echo '<a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&id='.urlencode(base64_encode($row['TransCode'])).'&v='.urlencode(base64_encode("view_log")).'" title="View Transaction Logs" class="btn btn-success" style="padding:4px;margin:4px;"> <i class="fa   fa-th-list fa-fw"></i></a>
																<a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&id='.urlencode(base64_encode($row['TransCode'])).'&v='.urlencode(base64_encode("update_transaction")).'" title="Update Transaction" class="btn btn-primary" style="padding:4px;margin:4px;"> <i class="fa  fa-pencil fa-fw"></i></a>
																
																<a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&id='.urlencode(base64_encode($row['TransCode'])).'&v='.urlencode(base64_encode("canceled_transaction")).'" title="Cancelled Transaction" class="btn btn-warning" style="padding:4px;margin:4px;"> <i class="fa  fa-trash-o fa-fw"></i></a>';
