...
?>
                ...
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-alerts">
                       ...
                       <?php
					   $mynotify=mysqli_query($con,"SELECT * FROM tbl_memo_notification INNER JOIN tbl_communication ON tbl_memo_notification.MemoNo = tbl_communication.MemoNo WHERE tbl_memo_notification.MemoTo='".$_SESSION['station']."'");
					   while($rownotify=mysqli_fetch_array( $mynotify))
					   {
						   if ($rownotify['MemoStatus']=='Unread')
						   {
							 echo '<li>
                            <a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&MemoNo='.urlencode(base64_encode($rownotify['MemoNo'])).'&v='.urlencode(base64_encode("memo_details")).'">
                                <div>
                        ...
                        echo '
                        <li>
                            <a class="text-center" href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&v='.urlencode(base64_encode("all_notification")).'">
                  ...
                  <!-- /Step Increment Generator -->			
		<?php
		$restep=mysqli_query($con,"SELECT * FROM tbl_deployment_history ORDER BY Date_assignment Asc");
		while($rowStep=mysqli_fetch_array($restep))
		{
			$totalYear=number_format($rowStep['No_of_years'] / $rowStep['StepNo'],0);
			$mystep=$rowStep['StepNo']+1;
			if ($totalYear==3)
			{
			 mysqli_query($con,"INSERT INTO tbl_messages VALUES(NULL,'PAYROLL','".$rowStep['Emp_ID']."','".'You are qualified for Step '.$mystep."','".date('Y-m-d')."','Unread','Steps')");
