                    ...
                    <div class="panel panel-default">
                       <div class="panel-heading">
					    <a href="#mytraining" class="btn btn-primary" style="float:right;" data-toggle="modal">New Training</a>
							...
							mysqli_query($con,"INSERT INTO tbl_seminar VALUES('".$_POST['Tcode']."','".$dataA."','".$_POST['TFrom']."','".$_POST['TTo']."','".$_POST['TConduct']."','".$_POST['TVenue']."','-','".$_POST['TOffice']."','-','Certificate','-')");
							...
							}elseif (isset($_POST['update-data']))
							{
								...
								mysqli_query($con,"UPDATE tbl_seminar SET Title_of_training='".$dataB."',covered_from='".$_POST['TFrom']."',covered_to='".$_POST['TTo']."',TVenue='".$_POST['TVenue']."' WHERE Training_Code='".$_SESSION['code']."' LIMIT 1");	
								if (mysqli_affected_rows($con)==1)
							{
								?>
							<script type="text/javascript">
                           ...
                           <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
									...
									$seminar=mysqli_query($con,"SELECT * FROM tbl_seminar WHERE Office='".$_SESSION['station']."'") or die ("error training data");
										...
										echo '<tr><td style="text-align:center;">'.$no.'</td>
												...
												<td class="dropdown">
												    <a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&code='.urlencode(base64_encode($row['Training_Code'])).'&v='.urlencode(base64_encode("view_list")).'" Title="View List of Participants" class="btn btn-success" style="padding:4px;margin:4px;"> <i class="fa fa-users fa-fw"></i></a>										 
												    <a href="./?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&code='.urlencode(base64_encode($row['Training_Code'])).'&v='.urlencode(base64_encode("other_list")).'" Title="View List of Speakers" class="btn btn-info" style="padding:4px;margin:4px;"> <i class="fa fa-user fa-fw"></i></a>										 
													 <a href="edit-training.php?'.$str.'7e9ff1f60111f1bf6a3696b2092ac4a7285cd942&code='.urlencode(base64_encode($row['Training_Code'])).'" data-toggle="modal" data-target="#editfile" title="Update Training" class="btn btn-warning" style="padding:4px;margin:4px;"> <i class="fa fa-search-minus fa-fw"></i></a>
													 <a style="cursor:pointer;padding:4px;margin:4px;" onclick="delete_me(this.id)" id="'.$row['Training_Code'].'" class="btn btn-danger" title="Remove Training"> <i class="fa fa-trash-o fa-fw"></i></a>										 
													 </td></tr>
												';
										}
													?>
