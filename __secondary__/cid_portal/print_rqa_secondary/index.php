...
$pdf->Image($img1,50,10,20);
	...
	$pdf->Cell(20,5,'RANK',1,1,'C');
	 ...
	 if ($rowsub['RankSubject']=='SCIENCE')
	{
	$myrank=mysqli_query($con,"SELECT * FROM tbl_rqa_list INNER JOIN tbl_applicant ON tbl_rqa_list.ApplicantNo = tbl_applicant.Appl_No WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Major LIKE '%".$rowsub['RankSubject']."%' ORDER BY tbl_rqa_list.Scores Desc");
	}else{	
	$myrank=mysqli_query($con,"SELECT * FROM tbl_rqa_list INNER JOIN tbl_applicant ON tbl_rqa_list.ApplicantNo = tbl_applicant.Appl_No WHERE tbl_applicant.Status <> 'OLD' AND tbl_applicant.Major='".$rowsub['RankSubject']."' ORDER BY tbl_rqa_list.Scores Desc");
	}
	while($newrank=mysqli_fetch_array($myrank))
	{
	    ...
	    $pdf->Cell(40,5,strtoupper($newrank['Major']),1,0,'C');		
