...
if (isset($voluntaryId)) {
    ...
    if ($voluntaryWork) {
        $voluntaryId = $voluntaryWork['id'];
        $organization = $voluntaryWork['organization'];
        $from = toDate($voluntaryWork['from_date'], 'Y-m-d');
        $isPresent = $voluntaryWork['is_present'] ? '1' : '0';
        $to = $isPresent ? date('Y-m-d') : toDate($voluntaryWork['to_date'], 'Y-m-d');
        $hours = $voluntaryWork['number_of_hours'];
        $position = $voluntaryWork['position_nature_of_work'];
    }
}
?>
        ...
        <form method="POST" action="">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label for="organization" class="mb-0">Name & Address of Organization (Write in full):
                        <?php showAsterisk() ?></label>
                    <textarea id="organization" name="organization" class="form-control" title="Required field" rows="3"
                        required><?= e($organization) ?></textarea>
                </div>
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-6" title="Check if present voluntary work">
                                    <input class="form-check-input" id="is-present" type="checkbox" name="is-present"
                                        value="1" <?= setItemChecked($isPresent) ?>>
                ...
                <div class="form-group">
                    ...
                    <input id="hours" type="number" name="hours" min="0" step="1" class="form-control"
                        title="Leave blank if not applicable" value="<?= e($hours) ?>">
                </div>
