        ...
        <div class="row">
            <div class="col-sm-12 col-md-10 col-lg-6 col-xl-4 col">
                    ...
                    <?php if ($isPis) : ?>
                        <select class="form-control" id="card-type" name="card-type" required>
                            ...
                            while ($type = fetchAssoc($cardTypes)) : ?>
                                <option value="<?php echo $type['id']; ?>" <?php echo setOptionSelected($type['id'], $cardType); ?>><?php echo $type['name']; ?></option>
                            <?php endwhile; ?>
                        </select>
                    <?php else : ?>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="card-number" name="card-number" value="<?php echo $number; ?>" <?php echo setActiveNavigation(!$isPis, 'readonly'); ?>>
                </div>

                <div class="form-group">
                    <label for="card-date" class="mb-0">Date of Issuance <?php if ($isPis) showAsterisk(); ?></label>
                    <input type="<?php echo $isPis ? 'date' : 'text'; ?>" class="form-control" id="card-date" name="card-date" value="<?php echo $date; ?>" <?php echo setActiveNavigation(!$isPis, 'readonly'); ?>>
                </div>

                <div class="form-group">
                    <label for="card-place" class="mb-0">Place of Issuance <?php if ($isPis) showAsterisk(); ?></label>
                    <input type="text" class="form-control" id="card-place" name="card-place" value="<?php echo $place; ?>" <?php echo setActiveNavigation(!$isPis, 'readonly'); ?>>
                </div>
