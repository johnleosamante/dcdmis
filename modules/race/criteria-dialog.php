...
if ($mode === 'view'):
    ...
    foreach ($criteria as $cr) {
        $totalMax += floatval($cr['max_points']);
    }
    ?>
            ...
            <form action="" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-light border mb-4">
                        <div class="d-flex align-items-start">
                            ...
                            <div>
                                ...
                                <div class="small text-muted">Select criteria from the shared list and assign the maximum
                                    points. You may add a new criterion if it is not listed.</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if (!empty($existingCriteria)): ?>
                        <div class="card border mb-4">
                            <div class="card-header bg-light py-3">
                                <h6 class="mb-0 font-weight-bold text-dark"><i class="fas fa-pen-to-square fa-fw mr-1"></i>
                                    Current Criteria for this Award</h6>
                            </div>
                            <div class="table-responsive">
                                        ...
                                        <?php foreach ($existingCriteria as $cr): ?>
                                            <tr id="criterion-row-<?= e($cr['id']) ?>">
                                                ...
                                                <td class="align-middle text-center">
                                                    <span
                                                        class="criterion-points-display font-weight-bold"><?= e(number_format($cr['max_points'], 2)) ?></span>
                                                    <input type="number"
                                                        class="form-control form-control-sm text-center criterion-points-edit d-none"
                                                        value="<?= e($cr['max_points']) ?>" min="0" step="any">
                                                </td>
                                                <td class="align-middle text-center">
                                                    <button type="button" class="btn btn-sm btn-outline-primary criterion-edit-btn"
                                                        onclick="
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-success criterion-save-btn d-none"
                                                        onclick="
                                                        var row=this.closest('tr');
                                                        var id='<?= e($cr['id']) ?>';
                    ...
                    <?php if (!empty($criteriaLibrary)): ?>
                        <div class="card border mb-4">
                            <div class="card-header bg-light py-3">
                                <h6 class="mb-0 font-weight-bold text-dark"><i class="fas fa-list-check fa-fw mr-1"></i>
                                    Available Criteria</h6>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover mb-0">
                                    <thead class="thead-light">
                                        ...
                                        <?php foreach ($criteriaLibrary as $libraryCriterion):
                                            ...
                                            <tr>
                                                <td class="text-center align-middle">
                                                    <input type="checkbox" class="position-static" name="library_criterion_id[]"
                                                        value="<?= e($libraryCriterion['id']) ?>" <?= $awardCriterion ? 'checked' : '' ?> aria-label="Use <?= e($libraryCriterion['criterion_name']) ?>">
                                                </td>
                                                ...
                                                <td class="align-middle">
                                                    <input type="number" class="form-control form-control-sm text-center"
                                                        name="library_max_points[<?= e($libraryCriterion['id']) ?>]"
                                                        value="<?= e($awardCriterion['max_points'] ?? $libraryCriterion['default_max_points']) ?>"
                                                        min="0" step="any"
                                                        aria-label="Maximum points for <?= e($libraryCriterion['criterion_name']) ?>">
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                    ...
                    <div class="card border">
                        <div class="card-header bg-light py-3 d-flex align-items-center justify-content-between">
                            <h6 class="mb-0 font-weight-bold text-dark"><i class="fas fa-plus-circle fa-fw mr-1"></i>
                                Additional Criteria</h6>
                        ...
                        <div class="card-body" id="criteria-list">
                            <?php if (!empty($customCriteria)): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p class="text-muted small mb-0">No additional criteria have been added. Use the button above to
                                    add one.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-light">
                    <?php cancelModalButton() ?>
                    <button class="btn btn-primary" name="save-ranking-criteria" type="submit"><i
                            class="fas fa-save fa-fw mr-1"></i> Save Criteria</button>
                </div>
            </form>
        </div>
    </div>
