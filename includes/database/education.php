...
function create_education($level, $school, $course, $from, $to, $highest, $year, $scholarship, $id) {
  non_query("INSERT INTO educational_background (`Level`, `Name_of_School`, `Course`, `From`, `To`, Highest_Level, Year_Graduated, Honor_Recieved, Emp_ID) VaLUES ('{$level}', '{$school}', '{$course}', '{$from}', '{$to}', '{$highest}', '{$year}', '{$scholarship}', '{$id}');");
}

function update_education($level, $school, $course, $from, $to, $highest, $year, $scholarship, $id, $no) {
  non_query("UPDATE educational_background SET `Level`='{$level}', Name_of_School='{$school}', Course='{$course}', `From`='{$from}', `To`='{$to}', Highest_Level='{$highest}', Year_Graduated='{$year}', Honor_Recieved='{$scholarship}' WHERE Emp_ID='{$id}' AND `No`='{$no}' LIMIT 1;");
}
