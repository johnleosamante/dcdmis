...
function completedDocuments($station) {
...
function receivedDocuments($station) {
...
function canceledDocuments($station) {
...
function updateDocumentLog($id, $user, $station, $destination, $purpose, $status='New', $details='', $change_date=true) {
  $date = $change_date ? "Date_Recieved=NOW(), " : '';
  nonQuery("UPDATE tbl_transactions_log SET " . $date . " Recieved_by='{$user}', From_office='{$station}', Forwarded_to='{$destination}', Trans_status='{$purpose}', `Status`='{$status}', `details`='{$details}' WHERE Transaction_code='{$id}' ORDER BY Date_Recieved DESC LIMIT 1;");
}
