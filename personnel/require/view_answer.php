...
<div class="panel panel-default">
		...
		Learner's Answer
			...
			<?php
			//Learner information
			$learner=mysqli_query($con,"SELECT * FROM tbl_student WHERE lrn='".$_GET['lrn']."' LIMIT 1");
			$rowlearner=mysqli_fetch_assoc($learner);
			//Learner Score
			$myscore1=mysqli_query($con,"SELECT * FROM tbl_activity_learner_score WHERE lrn='".$_GET['lrn']."' AND SubCode ='".$_SESSION['SubCode']."' AND Activity_Code='".$_SESSION['ActivityCode']."' LIMIT 1");
			$rowscore1=mysqli_fetch_assoc($myscore1);
			echo '<label style="padding:4px;margin:4px;">LRN: </label><label>'.$_GET['lrn'].'</label><br/>';
			echo '<label style="padding:4px;margin:4px;">Learner\'s Name: </label><label>'.$rowlearner['Lname'].', '.$rowlearner['FName'].'</label><br/>';
			echo '<label style="padding:4px;margin:4px;">Scores/Item: </label><label>'.$rowscore1['Score'].'/'.$rowscore1['Item'].'</label><hr/>';
			...
			<thead>
			<tbody>';
			$myanswer=mysqli_query($con,"SELECT * FROM tbl_activity_learner_answer WHERE LRN='".$_GET['lrn']."' AND SubCode='".$_SESSION['SubCode']."' AND Activity_Code='".$_SESSION['ActivityCode']."'");
			while($rowanswer=mysqli_fetch_array($myanswer))
			{
				$score=0;
				//Correct Answer
				$correct=mysqli_query($con,"SELECT * FROM tbl_activity_sheets WHERE SubCode ='".$_SESSION['SubCode']."' AND Activity_No='".$rowanswer['QNo']."' AND Activity_Code='".$_SESSION['ActivityCode']."'");
				$rowcorrect=mysqli_fetch_assoc($correct);
				echo '<tr>
						<td>'.$rowanswer['QNo'].'</td>
						<td>'.$rowanswer['Answer'].'</td>
						<td>'.$rowcorrect['Correct_Answer'].'</td>';
						if ($rowanswer['Answer']==$rowcorrect['Correct_Answer'])
						{
