...
$row=mysqli_fetch_assoc($result);
	...
	$family=mysqli_query($con,"SELECT * FROM family_background WHERE family_background.Emp_ID='".$_SESSION['EmpID']."' AND family_background.Relation='Wife' LIMIT 1");
	...
	$pdf->SetFont('Arial','',6);
	$pdf->Multicell(25,6,"SPOUSE'S SURNAME ",0,'L',true);
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($xPos+30,$yPos);
	$pdf->Multicell(76,6,$mydata['Family_Name'],1);
	$pdf->SetFont('Arial','',6);
	$pdf->SetXY($xPos+106,$yPos);
	...
	$xPos=$pdf->GetX();
	$yPos=$pdf->GetY();
	$pdf->Multicell(25,6,'FIRST NAME ',0,0,'L',true);
	$pdf->SetXY($xPos+25,$yPos);
	$pdf->Multicell(45,6,$mydata['First_Name'],1,'L');
	$pdf->SetXY($xPos+66,$yPos);
	$pdf->SetFont('Arial','',6);
	$pdf->Multicell(35,6,'NAME EXTENSION (JR., SR)',1,'L',true);
	$pdf->SetFont('Arial','',8);
	//$pdf->Cell(5,6,'',0,0,'C',true);
	$myson=mysqli_query($con,"SELECT * FROM family_background WHERE family_background.Emp_ID='".$_SESSION['EmpID']."' AND family_background.Relation<>'Wife'");
	while($mysonrow=mysqli_fetch_array($myson))
	{
	//$pdf->SetXY($xPos+101,$yPos);
	$pdf->cell(106,6,'',0,0,'C');	
	$pdf->cell(55,6,$mysonrow['Family_Name'].', '.$mysonrow['First_Name'],1,0);
	//$pdf->SetXY($xPos+156,$yPos);
	$pdf->cell(35,6,$mysonrow['Birthdate'],1,1);	
	
	}
	$xPos=$pdf->GetX();
	$yPos=$pdf->GetY();
	$pdf->Cell(5,6,'',0,0,'C',true);
	$pdf->Multicell(25,6,'MIDDLE NAME ',0,'L',true);
	$pdf->SetXY($xPos+30,$yPos);
	$pdf->Multicell(76,6,$mydata['Middle_Name'],1);	
	$pdf->Cell(5,6,'',0,0,'C',true);
	...
	$myeduc=mysqli_query($con,"SELECT * FROM educational_background WHERE educational_background.Emp_ID ='".$_SESSION['EmpID']."' ORDER BY educational_background.From Desc LIMIT 5");
	...
	$pdf->SetFont('Arial','',7);
	//getting data
	$myservice=mysqli_query($con,"SELECT * FROM civil_service WHERE civil_service.Emp_ID ='".$_SESSION['EmpID']."' LIMIT 10");
	...
	$pdf->SetFont('Arial','',7);
	//getting data
	$mywork=mysqli_query($con,"SELECT * FROM work_experience WHERE work_experience.Emp_ID ='".$_SESSION['EmpID']."'  ORDER BY work_experience.From Desc LIMIT 15");
	...
	$pdf->SetFont('Arial','',7);
	$myvolunt=mysqli_query($con,"SELECT * FROM voluntary_work WHERE voluntary_work.Emp_ID ='".$_SESSION['EmpID']."'  ORDER BY voluntary_work.From Desc LIMIT 5");
	...
	$pdf->SetFont('Arial','',7);
	$mylearn=mysqli_query($con,"SELECT * FROM learning_and_development WHERE learning_and_development.Emp_ID ='".$_SESSION['EmpID']."'  ORDER BY learning_and_development.From Desc LIMIT 9");
	...
	$myother=mysqli_query($con,"SELECT * FROM other_information WHERE other_information.Emp_ID ='".$_SESSION['EmpID']."' LIMIT 6");
	...
	$myreference=mysqli_query($con,"SELECT * FROM reference WHERE reference.Emp_ID ='".$_SESSION['EmpID']."' LIMIT 3");
	...
	$myValid=mysqli_query($con,"SELECT * FROM tbl_valid_id WHERE Emp_ID='".$_SESSION['EmpID']."' LIMIT 1")or die ("Error data");
	$getID=mysqli_fetch_assoc($myValid);
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(65,10,"Government Issued ID: ".$getID['Government'],1,1,'L');
	$pdf->Cell(65,10,"ID/License/Passport No.:  ".$getID['ID_Number'],1,1,'L');
	$pdf->Cell(65,10,"Date/Place of Issuance:  ".$getID['Place_issued'],1,1,'L');
	$pdf->SetXY($xPos+70,$yPos);
